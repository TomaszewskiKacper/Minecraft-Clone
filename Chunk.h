...
inline void Chunk<Depth, Width, Height>::Generate(const PerlinNoise& rng) {
	//define small increment step to smooth out noise, e.g. 0.01f
	const float incrementStep = 0.01f;
	//std::cout <<"grass tex:" << m_palette.LookUp(Cube::Type::Grass).Vbo() << std::endl;
	//std::cout <<"stone tex:" << m_palette.LookUp(Cube::Type::Stone).Vbo() << std::endl;



	for (size_t x = 0; x < Width; x++) {
		for (size_t z = 0; z < Depth; z++) {
			//define 2D point and transform it to world coords
			glm::vec2 worldPosition = glm::vec2(static_cast<float>(x), static_cast<float>(z));
			worldPosition += m_origin * glm::vec2(static_cast<float>(Width), static_cast<float>(Depth));
			//multiply it by increment step
			worldPosition *= incrementStep;

			//generate noise
			float noise = rng.At(glm::vec3(worldPosition.x, 0.0f, worldPosition.y));

			//column height
			float columnHeight = static_cast<float>(noise * Height);
			//std::cout << columnHeight<<std::endl;
			//fill coords with Cube up to height
			for (size_t y = 0; y < columnHeight-3; y++) {
				Cube::Type cubeType = Cube::Type::Stone;
				size_t index = CoordsToIndex(z, x, y);
				m_data[index].m_type = cubeType;
			}
			for (size_t y = columnHeight-3; y < columnHeight-1; y++) {
				Cube::Type cubeType = Cube::Type::Dirt;
				size_t index = CoordsToIndex(z, x, y);
				m_data[index].m_type = cubeType;
			}
			Cube::Type cubeType = Cube::Type::Grass;
			size_t index = CoordsToIndex(z, x, columnHeight);
			m_data[index].m_type = cubeType;
		}
	}
	UpdateVisibility();
